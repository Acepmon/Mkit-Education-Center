...
import student.launch.StudentUITest;
import student.login.controller.ClientTest;
import student.login.model.LoginFeature;
    ...
    private void Login() {
        ...
        if ((fld.getLength() <= 0) && (fld2.getLength() <= 0)) {
            ...
            if (res.equalsIgnoreCase("true")) {
                ...
                if (type.equalsIgnoreCase("оюутан")) {
                    // Undsen mainframe enchee duudaj og
                    tmp_username = fld.getText();
                    tmp_password = fld2.getText();
                    StudentUITest.Initialize();
                    LoginFeature.refreshFile();
                    System.out.println(CustomerLogin.savee);
                    

//                    test.Testing.setScene(new teacherMain().getScene());
                    StudentUITest.setScene(StudentUITest.mainFrame.getScene());
                } else {
//                  aa.setVisible(true);
                    aa.setVisible(false);
                    bb.setVisible(false);
                    pnl.setVisible(true);
                    ((Label) pnl.getChildren().get(0)).setText("Зөвхөн оюутан нэвтрэх ёстой!");
                }
