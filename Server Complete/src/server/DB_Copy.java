...
class Column {
    ...
    public void removeAt(int index) {
        ...
        MainClass.setOutputMessage("index: " + index);
...
/**
 * Host hiigdsen database iin buh table medeelel uudiig huulan avch server oor
 * deer hadgaldag.
 *
 * @author JAVA M2
 */
public class DB_Copy {
    ...
    public static ArrayList<Table> newTables = new ArrayList<>();
    ...
    public DB_Copy() {
        ...
        try {
            ...
            while (result.next()) {
                ...
                //      MainClass.setOutputMessage("" + r.getMetaData().getColumnCount());
               ...
               // MainClass.setOutputMessage("END");

            }
            MainClass.setOutputMessage("Successfully loaded all data from DB_Copy!!");
        } catch (SQLException ex) {
            ex.printStackTrace();
        }
        
        System.out.println("------");
        newTables.stream().forEach((Table t) -> System.out.println(t.getTableName()));
    }

    /**
     * Hadgalagdsan medeeleluudees tuhain table iin nertei medeeleliig ni shuuj
     * avna
     *
    ...
    public static void printNewTables() {
        for (Table table : newTables) {
            ...
            MainClass.setOutputMessage("Table name : " + table.getTableName());
            for (Column column : table.getDatas()) {
                System.out.println("Column name : " + column.getColumnName());
                MainClass.setOutputMessage("Column name : " + column.getColumnName());
                for (Object obj : column.getDatas()) {
                    System.out.println("Data: " + obj);
                    MainClass.setOutputMessage("Data: " + obj);
                }
            }
            System.out.println("\n\n");
            MainClass.setOutputMessage("\n\n");
        }
    }
    ...
    public static void insertRow(String tableName, ArrayList<Object> datas) {
        if (tableName != null && datas != null) {
            ...
            MainClass.setOutputMessage("Tableiin hemjee " + newTables.size());
            for (int i = 0; i < newTables.size(); i++) {
                System.out.println("table name -> " + newTables.get(i).getTableName() + " == " + tableName);
                MainClass.setOutputMessage("table name -> " + newTables.get(i).getTableName() + " == " + tableName);
                if (newTables.get(i).getTableName().equals(tableName)) {
                    if (newTables.get(i).getDatas().size() == datas.size()) {
                        ...
                        MainClass.setOutputMessage("-- insertRow -- \n Oruulj bui mor songogdson table iin baganatai tentsehgui baina.");
...
//            throw new NullPointerException();
    ...
    public static void removeAt(String tableName, int index) {
        MainClass.setOutputMessage("1-index: " + index);
        if (!tableName.equals("") && tableName != null) {
            for (int i = 0; i < newTables.size(); i++) {
